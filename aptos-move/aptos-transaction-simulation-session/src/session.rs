...
impl Session {
    ...
    /// Creates an account with proper AccountResource and funds it with APT.
    ///
    /// Unlike fund_account which only adds balance to the fungible store,
    /// this method creates the full account resource including authentication key.
    /// This is required for the account to be used as a transaction sender.
    pub fn create_and_fund_account(
        &mut self,
        account_address: AccountAddress,
        public_key: aptos_crypto::ed25519::Ed25519PublicKey,
        amount: u64,
    ) -> Result<()> {
        use aptos_transaction_simulation::{Account, AccountPublicKey};

        // Create an Account with the given address and public key
        let account =
            Account::new_from_addr(account_address, AccountPublicKey::Ed25519(public_key));

        // Use store_and_fund_account to create proper AccountResource
        let _account_data = self
            .state_store
        ...
        let summary_path = self
            .path
            .join(format!("[{}] fund (account)", self.config.ops))
            .join("summary.json");
        std::fs::create_dir_all(summary_path.parent().unwrap())?;
        std::fs::write(summary_path, serde_json::to_string_pretty(&summary)?)?;
    ...
    pub fn execute_transaction(
        ...
        let name = match &txn.payload() {
            ...
            TransactionPayload::Multisig(_multi_sig) => "multisig".to_string(),
            TransactionPayload::ModuleBundle(_) => unreachable!(),
            _ => "unknown".to_string(), // Payload and other future variants
        };
    ...
    pub fn execute_view_function(
        ...
        let (summary, res) = match output.values {
            Ok(values) => {
                // Convert raw values to JSON (as hex strings since view_function_returns
                // doesn't exist in this version of AptosValueAnnotator)
                let mut vals = Vec::new();
                for val in &values {
                    vals.push(serde_json::json!(hex::encode(val)));
                }
